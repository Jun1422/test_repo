...
######################Build the model use original dataset#####################################
   ...
   #build the formula
    ...
    saveformula = paste0("Surv(Tstart, Tstop, status) ~ ", formula, " + ",
                         exposure, ".3*", mediator, ".3 + strata(trans)")
    formula = as.formula(paste0("Surv(Tstart, Tstop, status) ~ ", formula, " + ",
                                exposure, ".3*", mediator, ".3 + strata(trans)"))}

    model_fit <- coxph(formula, data=msdata, method=method, model=TRUE)
...
#############################Prediction#########################################################
    ...
    ref_1 = ref
    ref_1[,exposure] = unique(data[,exposure])[which(unique(data[,exposure]) != unique(ref[,exposure]))]
    ...
    for (k in 1:nboot){
      ...
      ### Estimation
      fit <- coxph(formula, data=msbmt, method=method, model=TRUE)
      ### Prediction
      ...
      for (i in 1:length(grid)){
        ...
        #newd0 <- merge(newd_update,newd_grid, all=F)
        ...
        SDE[i] <- (Prg00E+Prg01E)-(Pr00e+Pr01e)
      ...
      est = rbind(est,cbind(TE,SDE,TE-SDE))
    }
    ...
    names(est) = c("TE","SDE","SIE")
    est = est[-1,]
    est$time_grid=rep(grid, nboot)
    est = est[c("time_grid","TE","SDE","SIE")]
 ######################Prepare output##############################################################
    out <- list()
    method_list <- list()
    variable_list <- list()
    ...
    results <- est %>% group_by(time_grid) %>% summarise(sd_te = sd(TE),
                                                         ...
                                                         se_te = sd_te/nboot,
                                                         se_sde = sd_sde/nboot,
                                                         se_sid = sd_sie/nboot) %>% as.data.frame()
    out$results = results

    te = quantile(sort(est$TE[est$time_grid == survival_time_fortable]),c(0.025,0.975))
    sde = quantile(sort(est$SDE[est$time_grid == survival_time_fortable]),c(0.025,0.975))
    sie = quantile(sort(est$SIE[est$time_grid == survival_time_fortable]),c(0.025,0.975))
    ci=data.frame(cbind(TE=paste0("(",round(te[1],4),", ",round(te[2],4),")"), SDE=paste0("(",round(sde[1],4),", ",round(sde[2],4),")"), SIE=paste0("(",round(sie[1],4),", ",round(sie[2],4),")")))
...
#the point estimator for certain time point(0-max); the user should specify the time intervals, width for grid,
    ...
    summary_table = rbind(est %>%filter(time_grid == survival_time_fortable) %>%
      ...
      summarise(TE = round(mean(TE),4),
                SDE = round(mean(SDE),4),
                SIE = round(mean(SIE),4)) %>% select(TE,SDE,SIE)
    %>% as.data.frame(), ci)
    rownames(summary_table) = c('Point estimate', '95% CI')
    out$summary_table = summary_table

    point_effect = results %>% filter(time_grid == survival_time_fortable) %>% select(TE,SDE,SIE)
    out$effect.pe = point_effect

    point_effect_sr = results %>% filter(time_grid == survival_time_fortable) %>% select(se_te,se_sde,se_sid)
    out$effect.se = point_effect_sr

    point_ci_low = c(te[1],sde[1],sie[1])
    names(point_ci_low) = c('TE','SDE','SIE')
    out$effect.ci.low = point_ci_low

    point_ci_high = c(te[2],sde[2],sie[2])
    names(point_ci_high) = c('TE','SDE','SIE')
