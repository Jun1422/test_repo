...
#####restrictions on data
 ...
 if(length(unique(data[,exposure]))!=2) stop("Binary exposure required")
 if(!is.factor(data[,exposure])) stop("Exposure must be a factor with 2 levels")
...
ggcmest <- function(x, errorbar.width = 0.3, errorbar.size = 0.3, errorbar.colour = "black",
  ...
  #multistate model
   if(x$methods$model == 'Multistate'){
     est_long = gather(x$results %>% select(time_grid,TE,SDE,SIE), effect, est, c("TE", "SDE", "SIE"), factor_key=T)
     cols <- c("TE" = "#332288",
               "SDE" = "#648FFF",
               "SIE" = "#DC267F")
